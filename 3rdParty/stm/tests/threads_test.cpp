...
namespace {
	enum { thread_count = 2, iterations = 50000 };
	...
	void verify(stm::transaction& tx) {
		...
		REQUIRE(-a == thread_count * iterations);
		REQUIRE(b == thread_count * iterations);
	}
...
TEST_CASE("threads", "misc threaded tests")
{
	SECTION("short-rw-transactions", "Run a number of short modifying transactions concurrently")
	{
		...
		REQUIRE(stm::detail::open_shared(val1).lock_val(0) == 0);
		REQUIRE(stm::detail::open_shared(val1).lock_val(1) == 0);
		REQUIRE(stm::detail::open_shared(val2).lock_val(0) == 0);
		REQUIRE(stm::detail::open_shared(val2).lock_val(1) == 0);
	...
	SECTION("short-r-rw-transactions", "Run a short modifying and nonmodifying transactions concurrently")
	{
		stm::shared<int> val1(0);
		stm::shared<int> val2(0); 
		...
		REQUIRE(stm::detail::open_shared(val2).lock_val(0) == 0);
		REQUIRE(stm::detail::open_shared(val2).lock_val(1) == 0);
		REQUIRE(stm::detail::open_shared(val3).lock_val(0) == 0);
		REQUIRE(stm::detail::open_shared(val3).lock_val(1) == 0);
		REQUIRE(stm::detail::open_shared(val4).lock_val(0) == 0);
		REQUIRE(stm::detail::open_shared(val4).lock_val(1) == 0);
	}

	SECTION("short-r-rw-on-shared-and-detached", "Run a mix of transactions on a mix of regular and detached")
	{
		int i[2] = {};
	...
	SECTION("short-readers", "short read-only transactions")
	{
		stm::shared<int> val1(0);
		stm::shared<int> val2(0); 
