...
//			});
//
//			data.insert(0, std::make_pair(thread_num, static_cast<int>((res.commits * 1000) / elapsed)));
//			data.insert(1, std::make_pair(thread_num, static_cast<int>(((res.open_failures + res.commit_lock_failures + res.commit_validate_failures) * 1000) / elapsed)));
//			data.insert(2, std::make_pair(thread_num, static_cast<int>((res.open_failures * 1000) / elapsed)));
//			data.insert(3, std::make_pair(thread_num, static_cast<int>((res.commit_lock_failures * 1000) / elapsed)));
//			data.insert(4, std::make_pair(thread_num, static_cast<int>((res.commit_validate_failures * 1000) / elapsed)));
//
//			data.insert(5, std::make_pair(thread_num, static_cast<int>((write_stats.commits * 1000) / elapsed)));
//			data.insert(6, std::make_pair(thread_num, static_cast<int>(((write_stats.open_failures + write_stats.commit_lock_failures + write_stats.commit_validate_failures) * 1000) / elapsed)));
//			data.insert(7, std::make_pair(thread_num, static_cast<int>((write_stats.open_failures * 1000) / elapsed)));
//			data.insert(8, std::make_pair(thread_num, static_cast<int>((write_stats.commit_lock_failures * 1000) / elapsed)));
//			data.insert(9, std::make_pair(thread_num, static_cast<int>((write_stats.commit_validate_failures * 1000) / elapsed)));
//
//			return *this;
//		}
//
...
struct bench_data {
    std::vector<stm::atomic_detail> primary;
    std::vector<stm::atomic_detail> secondary;
    double elapsed_time;
};

void bench_new(size_t tx_size) {
    // want to be able to specify parameters at top-level
    // is there anything else we can factor out?
    
    std::map<size_t, benchmark> bms;
    
    std::vector<stm::shared<int>> val(tx_size, stm::shared<int>(42));
    
    for (size_t num_readers = 1; num_readers != 4; ++num_readers) {
        benchmark bm;
        bm([&](stm::atomic_detail& atomic){
            atomic([&](stm::transaction& tx) {
                std::for_each(val.begin(), val.end(), [&tx](stm::shared<int>& v) {v.open_rw(tx); });
            });
        }, 1, [&](stm::atomic_detail& atomic){
            atomic([&](stm::transaction& tx) {
                std::for_each(val.begin(), val.end(), [&tx](stm::shared<int>& v) {v.open_r(tx); });
            });
    ...
    // what do we intend to draw here?
    // simple throughput (commits/s)
    //  x: num_threads, y: (writer commits, accum reader commits)
    // stats:
    
    // pass bms + annotations to some chart func
    // or possibly chart(bms, cookfunc)
}

STM_BENCHMARK(read_write)
{
//	basic_s();
//	basic_l();
//	stats_s();
