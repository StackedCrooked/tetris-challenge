...
namespace stm {
	namespace frontend {
		...
		struct shared_internal : public shared_internal_common<shared_internal<T>, T >
		{
			...
			template <typename U>
			explicit shared_internal(const shared_internal<U>& other) : val0(other.atomic([&](stm::transaction& tx) { return other.open_r(tx); })), val1(std::move(val0)) {
			...
			template <typename buf_traits>
			assign_type get_assign_func(slot_offset_t slot_id) {
				return (slot_id == 0)? static_cast<assign_type>(assign<T, buf_traits, shared_internal<T>, 1>) : static_cast<assign_type>(assign<T, buf_traits, shared_internal<T>, 0>);
			}
