	...
	namespace frontend {
      ...
      // "flushes" changes, bringing the attached object up to date before leaving it
			...
			const T& operator[](slot_offset_t id) const {
				return  (id == 0) ? static_cast<const T&>(*primary) : secondary;
			}

			template <typename buf_traits>
			assign_type get_assign_func(slot_offset_t slot_id) {
				return (slot_id == 0)? static_cast<assign_type>(assign<T, buf_traits, shared_internal_detached<T>, 1>) : static_cast<assign_type>(assign<T, buf_traits, shared_internal_detached<T>, 0>);
			}
