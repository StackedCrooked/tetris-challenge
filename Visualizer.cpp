...
Visualizer::Visualizer(const GameGrid & inGameGrids) :
...
void Visualizer::onPaint(HDC inHDC)
{
	...
	for (size_t rowIdx = 0; rowIdx != mGameGrid.grid().numRows(); ++rowIdx)
	{
		...
		{
			Gdiplus::SolidBrush fgBrush(GetColor(static_cast<Block::Type>(mGameGrid.grid().get(rowIdx, colIdx))));
			Gdiplus::RectF rect
			(
...
Gdiplus::Color Visualizer::GetColor(Block::Type inType)
{	
	switch (inType)
	{
		case Block::I_BLOCK:
