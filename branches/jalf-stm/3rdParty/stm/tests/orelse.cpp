...
TEST_CASE("orelse", "orelse tests")
{
    ...
    SECTION("orelse-inner-nested-fail", "If a transaction fails, and its ancestor (but not immediate parent) is an orelse, the orelse should be triggered")
    {
        bool b = false;
        stm::atomic(stm::orelse(
                                [](stm::transaction&){stm::atomic([](stm::transaction& tx){ tx.retry(); });},
                                [&b](stm::transaction&){b = true;}));
        REQUIRE(b);
    }
}
