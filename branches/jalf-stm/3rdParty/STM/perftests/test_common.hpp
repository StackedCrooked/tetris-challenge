...
enum { default_test_duration = 2000, iters = 5, big_tx = 500 };
...
struct repeat_while {
	repeat_while(uint32_t ms) : end(timer_timestamp() + ticks_per_ms * ms), iters() {}
	bool operator()() {
		++iters;
		return timer_timestamp() < end;
	}

	uint64_t iterations() throw() { return iters; }
...
struct repeat_while2 {
	repeat_while2 (uint32_t ms) : ms(ms), stop() {}
	bool operator()() { return stop == 0l; }
	uint64_t block() {
		auto start = timer_timestamp();
#ifdef _MSC_VER
		Sleep(ms);
		_InterlockedIncrement(&stop);
#else
		timespec ts;
		ts.tv_sec = 0;
		ts.tv_nsec = static_cast<unsigned long>(ms) * 1000000ul;
		nanosleep(&ts, NULL);
		__sync_fetch_and_add (&stop, 1);
#endif
		return (timer_timestamp() - start) / ticks_per_ms;
	}
private:
	uint32_t ms;
	long stop;
