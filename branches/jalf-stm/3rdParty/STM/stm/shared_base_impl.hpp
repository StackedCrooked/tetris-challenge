...
namespace stm {
	namespace backend {
		...
		STM_LIB_FUNC slot_offset_t shared_base::acquire_for_read(version_field_t tx_version){ // todo: lock_for_commit really needs the same but for other slot. Can we unify? (note a few subtle active/inactive and !=/== diffs
			version_field_t hdr = header();
		...
		STM_LIB_FUNC void shared_base::release_reader(slot_offset_t slot_id) throw() {
